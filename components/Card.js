...
const Card = (props) => {
  ...
  return (
    <>
      <div className='card hover:bg-[#f3f7ff] w-[30%] h-[64vh] border-2 border-slate-300 rounded-3xl mb-16 p-3 overflow-x-hidden overflow-y-hidden '>
        ...
        <div className="tech flex gap-2 w-full  min-h-12 h-fit flex-wrap">

          {/* note below how we use map function that means return div directly not any <></> */}
          {techarr !== 0 && techarr.map(obj => {
            return <div key={obj.id} className="techbtn border-slate-400 border-[1px] bg-slate-300 w-fit h-fit rounded-md px-1">
              {obj.item}
            </div>
          })}
